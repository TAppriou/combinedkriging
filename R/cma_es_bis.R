cma_es_bis <- function(par, fn, ..., lower, upper, control = list()) {
  ...
  while (iter < maxiter) {
    ...
    # arfitness <- y * pen
    # valid <- pen <= 1
    arfitness <- y / pen #Changed since the objective to minimize -EI is negative in our case
    valid <- pen <= Inf #Changed to avoid cases where no point in the population is valid
    if (any(valid)) {
      wb <- which.min(y[valid])
      if (y[valid][wb] < best.fit) {
        best.fit <- y[valid][wb]
        # best.par <- arx[, valid, drop = FALSE][, wb]
        best.par <- vx[, valid, drop = FALSE][, wb] #Changed to make sure that this point is in the box defined by upper and lower
      }
    }
